...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = stores.values.reduce([]){ |acc, cur| acc + cur[:employees] }
  ...
  it 'test 8' do
    ...
    ingredients = stores.values.reduce([]){ |acc, cur|
      acc + cur[:dishes].reduce([]){ |acc, cur|
        acc + cur[:ingredients]
      }
    }
  ...
  it 'test 9' do
    ...
    full_menu_price = stores[:olive_garden][:dishes].reduce(0){ |acc, cur| acc + cur[:price] }
  ...
  it 'test 10' do
    ...
    olive_garden_menu = stores[:olive_garden][:dishes].reduce({}){ |acc, cur| { **acc, cur[:name] => cur } }
  ...
  it 'test 11' do
    ...
    full_menu = stores.values.reduce({}){ |acc, cur|
